...
interface RecordItem {
    ...
    role: string;
    status: string;
    author: string; // email (used under Name)
    createdBy?: string;
    date: string;
}
...
export default function DataTable({
    ...
    const [visibleCols, setVisibleCols] = useState<Record<keyof RecordItem | "select" | "actions", boolean>>({
        ...
        role: true,
        status: true,
        author: false, // no separate Author column
        createdBy: true,
        date: true,
        actions: true,
    });
    ...
    const filtered = useMemo(() => {
        let out = rows?.filter((r) =>
            [r.id, r.title, r.author, r.role, r.createdBy].join(" ").toLowerCase().includes(debouncedQuery.toLowerCase())
        );
        out?.sort((a, b) => {
    ...
    const exportCSV = () => {
        const cols: (keyof RecordItem)[] = ["id", "title", "role", "status", "createdBy", "date"];
                    ...
                    <Table className="text-[0.94rem]">
                        <TableHeader>
                            <TableRow className="sticky top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
                                ...
                                {visibleCols.role && (
                                    <TableHead aria-sort={sortKey === "role" ? (sortDir === "asc" ? "ascending" : "descending") : "none"} className="cursor-pointer select-none" onClick={() => toggleSort("role")}>
                                        Role {sortIndicator("role")}
                                    </TableHead>
                                )}
                                {visibleCols.status && <TableHead>Status</TableHead>}
                                {visibleCols.createdBy && (
                                    <TableHead aria-sort={sortKey === "createdBy" ? (sortDir === "asc" ? "ascending" : "descending") : "none"} className="cursor-pointer select-none" onClick={() => toggleSort("createdBy")}>
                                    ...
                                    {visibleCols.title && (
                                        <TableCell>
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[12px] text-neutral-500">{r.author}</span>
                                            </div>
                                        </TableCell>
                                    )}
