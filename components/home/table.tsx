...
import type { RecordItem } from "."; // from home index
...
export default function HomeTable({
    ...
    const exportCSV = () => {
        const cols: (keyof RecordItem)[] = ["id", "siteName", "title", "author", "date", "brandsCount", "servicesCount"];
    ...
    const renderThumb = (r: RecordItem) => {
        const src = r.logoImage || r.hero || null;
        if (!src) {
            return (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative w-[260px] max-w-[50vw]">
                            <Input
                                ...
                                placeholder="Search home settings…"
                                className="h-9 pl-9"
                            />
                    ...
                    <Table className="text-[0.94rem]">
                        <TableHeader>
                            <TableRow className="sticky top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
                                ...
                                {visibleCols.siteName && (
                                    <TableHead
                                        aria-sort={sortKey === "siteName" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("siteName")}
                                    >
                                        Site Name {sortIndicator("siteName")}
                                    </TableHead>
                                )}
                                ...
                                {visibleCols.brandsCount && (
                                    <TableHead
                                        aria-sort={sortKey === "brandsCount" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("brandsCount")}
                                    >
                                        Brands {sortIndicator("brandsCount")}
                                    </TableHead>
                                )}

                                {visibleCols.servicesCount && (
                                    <TableHead
                                        aria-sort={sortKey === "servicesCount" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("servicesCount")}
                                    >
                                        Services {sortIndicator("servicesCount")}
                                    </TableHead>
                                )}
                        ...
                        <TableBody>
                            {current.map((r, i) => (
                                ...
                                >
                                    ...
                                    {visibleCols.siteName && (
                                        <TableCell className="font-medium text-[#B5040F]">{r.siteName}</TableCell>
                                    )}
                                    ...
                                    {visibleCols.description && (
                                        <TableCell>
                                            <span className="line-clamp-1 max-w-[200px]">{r.description || <span className="text-neutral-400">—</span>}</span>
                                        </TableCell>
                                    )}
                                    ...
                                    {visibleCols.brandsCount && <TableCell>{r.brandsCount ?? 0}</TableCell>}

                                    {visibleCols.servicesCount && <TableCell>{r.servicesCount ?? 0}</TableCell>}
