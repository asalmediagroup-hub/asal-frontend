...
import type { PublishStatus, NewsDoc } from "@/slices/newsApi";
import { useGetNewsByIdQuery } from "@/slices/newsApi";

type ImageMode = "keep" | "file" | "url" | "clear";
const STATUSES: PublishStatus[] = ["draft", "published"];

export interface NewsItemForm {
    date?: string;                // ISO date string (YYYY-MM-DD)
    author?: string;
    title: string;
    image?: File | string | null; // File for upload, or URL, or null to clear
    description?: string;
    fullNews: string;
    order?: number;
}

/** This matches RecordItem from news/index plus extra fields when editing */
...
export default function NewsModal({ open, onOpenChange, initial, onSave, onDelete }: NewsModalProps) {
    // ---------------- form state ----------------
    ...
    // fetch full news when editing to seed all fields
    const newsId = initial?.id || "";
    const { data: newsData } = useGetNewsByIdQuery(newsId, { skip: !newsId });

    // seed from initial / newsData
    useEffect(() => {
        if (initial) {
            const src = (newsData as NewsDoc | undefined) ?? (initial as any);
            ...
            // Map server items (URLs) to form items (image stays string URL; can be replaced with File)
            const serverItems = (src as any).items as
                | { date?: string; author?: string; title: string; image?: string | null; description?: string; fullNews: string; order?: number }[]
                | undefined;

            setItems(
                Array.isArray(serverItems)
                    ? serverItems.map((it) => ({
                        date: normalizeDateForInput(it?.date),
        ...
        const payload = {
            ...
            items: (items ?? [])
                .map((it) => ({
                    ...
                    author: String(it?.author ?? "News Desk").trim() || "News Desk",
                    title: String(it?.title ?? "").trim(),
                    image: normalizeImage(it?.image), // File | string | null
                    description: String(it?.description ?? "").trim(),
                    fullNews: String(it?.fullNews ?? "").trim(),
                    order: Number(it?.order) || 0,
                }))
            ...
            <DialogContent className="p-0 sm:max-w-3xl overflow-hidden">
                <form onSubmit={handleSave} className="flex max-h-[80vh] flex-col">
                    ...
                    <DialogHeader className="sticky top-0 z-10 space-y-1 border-b bg-white px-6 py-4">
                        <DialogTitle className="text-lg">{initial ? "Edit news" : "Create news"}</DialogTitle>
                                ...
                                {(item, update, index, duplicate, remove) => (
                                    <div className="rounded-lg border border-neutral-300 p-3 space-y-3">
                                        <div className="grid grid-cols-12 gap-3">
                                            ...
                                            <div className="col-span-12">
                                                <Field label="Full News (HTML/text)">
                                                    <Textarea
                                                        rows={5}
                                                        className="border border-neutral-300"
                                                        value={item.fullNews ?? ""}
                                                        onChange={(e) => update({ fullNews: e.target.value })}
                                                        placeholder="<p>Full body...</p>"
                                                    />
                                                </Field>
                                            </div>
...
function ItemImageField({
    ...
    item: NewsItemForm;
    update: (patch: Partial<NewsItemForm>) => void;
}) {
...
function isCompletelyEmptyItem(it: {
    ...
    const noAuthor = !it.author || it.author === "News Desk";
    const noTitle = !it.title;
    const noDesc = !it.description;
    const noFull = !it.fullNews;
    const noImg = !it.image;
    const noOrder = !it.order || Number(it.order) === 0;
    return noDate && noAuthor && noTitle && noDesc && noFull && noImg && noOrder;
}
