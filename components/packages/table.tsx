...
import type { RecordItem } from "."; // from packages index page
...
export default function PackageTable({
    ...
    const [sortKey, setSortKey] = useState<keyof RecordItem | "storiesCount">("title");
    const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");

    const [selected, setSelected] = useState<Set<string>>(new Set());

    const [visibleCols, setVisibleCols] = useState<
        Record<keyof RecordItem | "select" | "actions" | "imageCol" | "storiesCount", boolean>
    >({
        select: true,
        title: true,
        slug: true,
        ...
        imageCol: true,
        actions: true,
        // optional/hidden to reduce noise
        description: false,
        id: false,
        featuredStories: false,
    });

    const toggleSort = (key: keyof RecordItem | "storiesCount") => {
    ...
    const sortIndicator = (key: keyof RecordItem | "storiesCount") => (
        <ArrowUpDown
            className={cn(
    ...
    const filtered = useMemo(() => {
        ...
        out.sort((a, b) => {
            if (sortKey === "storiesCount") {
                const A = Array.isArray((a as any).featuredStories) ? (a as any).featuredStories.length : 0;
                const B = Array.isArray((b as any).featuredStories) ? (b as any).featuredStories.length : 0;
    ...
    const exportCSV = () => {
        const cols: (keyof RecordItem | "storiesCount")[] = [
            "id",
            "title",
            "slug",
            "category",
        ...
        const rowsToExport = src.map((r) => {
            const count = Array.isArray((r as any).featuredStories) ? (r as any).featuredStories.length : 0;
            const get = (key: keyof RecordItem | "storiesCount") =>
                key === "storiesCount" ? String(count) : String(((r as any)[key] ?? "")).replace(/"/g, '""');
            return cols.map((c) => get(c)).join(",");
        });
    ...
    const renderThumb = (r: RecordItem) => {
        // show first featured story image if available
        const first = Array.isArray((r as any).featuredStories) ? (r as any).featuredStories[0] : null;
        const src = first?.image || null;
        if (!src) {
            return (
                    ...
                    <Table className="text-[0.94rem]">
                        <TableHeader>
                            <TableRow className="sticky top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
                                ...
                                {visibleCols.storiesCount && (
                                    <TableHead
                                        aria-sort={sortKey === "storiesCount" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("storiesCount")}
                                    >
                                        <span className="inline-flex items-center">
                                            <BookOpen className="mr-1 h-4 w-4" />
                                            Stories {sortIndicator("storiesCount")}
                                        </span>
                                    </TableHead>
                                )}
                        ...
                        <TableBody>
                            {current.map((r, i) => {
                                const storiesCount = Array.isArray((r as any).featuredStories) ? (r as any).featuredStories.length : 0;
                                return (
                                    <TableRow
                                        key={r.id}
                                    ...
                                    >
                                        ...
                                        {visibleCols.slug && (
                                            <TableCell>{(r as any).slug || <span className="text-neutral-400">—</span>}</TableCell>
                                        )}

                                        {visibleCols.category && (
                                            <TableCell>{(r as any).category || <span className="text-neutral-400">—</span>}</TableCell>
                                        )}

                                        {visibleCols.status && (
                                            <TableCell>
                                                <StatusBadge status={r.status as "draft" | "published"} />
                                            </TableCell>
                                        )}
                                        ...
                                        {visibleCols.storiesCount && <TableCell>{storiesCount}</TableCell>}
