...
import type { RecordItem } from "."; // from portfolio index
...
export default function PortfolioTable({
    ...
    const [sortKey, setSortKey] = useState<keyof RecordItem>("title");
    ...
    >({
        select: true,
        title: true,
        author: true,        // if provided by index
        date: true,
        itemsCount: true,
        firstItemTitle: false,
        description: false,
        imageCol: true,
        actions: true,
        // hidden internals:
        firstItemImage: false,
        id: false,
    });
    ...
    const filtered = useMemo(() => {
        ...
        out.sort((a, b) => {
            ...
            // date sort should be chronological
            if (sortKey === "date") {
                const aT = new Date(String(A)).getTime() || 0;
                const bT = new Date(String(B)).getTime() || 0;
    ...
    const exportCSV = () => {
        const cols: (keyof RecordItem)[] = ["id", "title", "author", "date", "itemsCount", "firstItemTitle"];
    ...
    const renderThumb = (r: RecordItem) => {
        const src = r.firstItemImage || null;
        if (!src) {
            return (
                        ...
                        <TableHeader>
                            <TableRow className="sticky top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
                                ...
                                {visibleCols.itemsCount && (
                                    <TableHead
                                        aria-sort={sortKey === "itemsCount" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("itemsCount")}
                                    >
                                        Items {sortIndicator("itemsCount")}
                                    </TableHead>
                                )}

                                {visibleCols.firstItemTitle && (
                                    <TableHead
                                        aria-sort={sortKey === "firstItemTitle" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
                                        className="cursor-pointer select-none"
                                        onClick={() => toggleSort("firstItemTitle")}
                                    >
                                        First Item {sortIndicator("firstItemTitle")}
                                    </TableHead>
                                )}
                        ...
                        <TableBody>
                            {current.map((r, i) => (
                                ...
                                >
                                    ...
                                    {visibleCols.author && (
                                        <TableCell>
                                            <span className={cn(!(r as any).author && "text-neutral-400")}>
                                                {(r as any).author || "—"}
                                            </span>
                                        </TableCell>
                                    )}
                                    ...
                                    {visibleCols.itemsCount && <TableCell>{r.itemsCount ?? 0}</TableCell>}

                                    {visibleCols.firstItemTitle && (
                                        <TableCell>{r.firstItemTitle || <span className="text-neutral-400">—</span>}</TableCell>
                                    )}
